	...
	def staffdetails(self):
	    ...
	    a=="y":
	    	...
	    	con=cx_Oracle.connect("nouffel/nouffel@127.0.0.1/XE")
	            cur=con.cursor()
                        sql = "insert into staffdetails VALUES('%s','%s','%s','%s','%s','%s')" % (self.car1,self.cs,self.van1,self.cs1,self.user1,self.pass1)
                        cur.execute(sql)
                        con.commit()
	    	print(1------upate 2----- delete)
	    	print("enter your choice")
	    	ch=str(input())
	    	if ch=="1":
	    		print("enter the vechile you to update car/van")
	    	
	    		print("enter the car name")
	    		cr=str(input())
	    		con=cx_Oracle.connect("nouffel/nouffel@127.0.0.1/XE")
		            cur=con.cursor()
                                    sql = "update staff set cr='%s' where user1='%s'" % (llp,cr)
            	            cur.execute(sql)
            	            con.commit()
			...
			cur=con.cursor()
	    		...
	    		print("enter the vechile you to delete car/van")
	    		print("enter the car name");
	    		mk=str(input())
	    		con=cx_Oracle.connect("nouffel/nouffel@127.0.0.1/XE")
		            cur=con.cursor()
                                    sql="delete from staff where user1='%s'"  % (mk)
                                    cur.execute(sql)
                                    con.commit()
	    	else
	    		print("enter the van name")
	    		...
	    		con=cx_Oracle.connect("nouffel/nouffel@127.0.0.1/XE")
	    	            ...
	    	            con=cx_Oracle.connect("nouffel/nouffel@127.0.0.1/XE")
	                        cur=con.cursor()
                                    sql = "insert into staffdetails VALUES('%s','%s','%s','%s','%s')" % ()
                                    cur.execute(sql)
                                    con.commit()
class account();
	def detils(self):
